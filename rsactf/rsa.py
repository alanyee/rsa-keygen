...
def _parse_args():
    ...
    parser.add_argument(
        "-m", "--message", type=str, help="Message to be encrypted and decrypted"
    )
    parser.add_argument(
        "-l",
        "--prime-length",
...
def main():
    ...
    for i in range(2):
        if args.primes[i] == 0:
            p_len = (
                args.prime_lengths[i] if args.prime_lengths else args.prime_length[0]
            )
            print(
                f"Generating {'1st' if i==0 else '2nd'} prime of length {p_len}",
                end=" ... ",
            )
